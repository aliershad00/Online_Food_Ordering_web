...
module.exports = {
  ...
  //not need async createIngredientsItem(
  //   restaurantId,
  //   ingredientName,
  ...
  async createIngredientsItem(
    ...
    try {
      ...
      // Create a new IngredientsItem instance
      item = new IngredientsItem({
        name: ingredientName,
        restaurant: restaurantId,
        category: category._id,
      ...
      const savedItem = await item.save(); // Save the item to the database

      // Add the saved item to the category's ingredients array
      category.ingredients.push(savedItem._id);
      await category.save();

      // Populate the category field before returning the saved item
